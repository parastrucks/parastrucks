...
export default function ProformaInvoice() {
  ...
  async function handleSubmit(e) {
      ...
      for (let ri = 0; ri < m.rows.length; ri++) {
        ...
        if (!r.chassis_no.trim() || !r.engine_no.trim()) {
          ...
          focusEl(document.getElementById(
            !r.chassis_no.trim() ? `pi-chassis-${m.id}-${r.id}` : `pi-engine-${m.id}-${r.id}`
          ))
          return
        }
        if (!(parseInt(r.mrp, 10) > 0)) {
          ...
          focusEl(document.getElementById(`pi-mrp-${m.id}-${r.id}`))
          return
        }
      }
    }
    ...
    try {
      ...
      toast.error(err.message || 'Failed to generate proforma invoices.')
    } finally {
      setSaving(false)
    }
  }
            ...
            <div className="q-section">
              ...
              <div className="customer-grid">
                <div className="form-group span-2" style={{ marginBottom: 0 }}>
                  ...
                  <input id="pi-cust-name" className={`form-input ${errorField === 'name' ? 'error' : ''}`} value={customer.name}
                ...
                <div className="form-group" style={{ marginBottom: 0 }}>
                  ...
                  <input id="pi-cust-gstin" className={`form-input ${errorField === 'gstin' ? 'error' : ''}`} value={customer.gstin}
            ...
            <div className="q-section">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <div style={{ position: 'relative', flex: 1 }} ref={searchRef}>
                  <input id="pi-search" className={`form-input ${errorField === 'search' ? 'error' : ''}`}
                  ...
                  {model.rows.map((row, ri) => (
                    <div key={row.id} className="model-chip-row">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        ...
                        <input id={`pi-chassis-${model.id}-${row.id}`}
                        ...
                        <input id={`pi-engine-${model.id}-${row.id}`}
                      ...
                      <div className="model-chip-row-prices">
                        <div className="form-group" style={{ marginBottom: 0 }}>
                          ...
                          <input id={`pi-mrp-${model.id}-${row.id}`}
