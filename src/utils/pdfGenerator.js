...
// ─────────────────────────────────────────────────────────────────────────
// Phase 9.5 — Service Team outside-workshop job PO / warranty-request letter.
// Two layouts branch on (job_type, repair_type):
//   • Ancillary·Warranty → warranty-request letter ("To, Authorized Dealer").
//   • all other three     → conventional vendor purchase order.
// No amounts (v1). Entity header/footer sourced from the entities row (never
// hard-coded) + isPT stamp. A "PO generated by {name}" line sits at the bottom.
// ─────────────────────────────────────────────────────────────────────────

// Local date formatter for date-only ('YYYY-MM-DD') values — avoids the
// new Date(isoString) UTC-rollback that PR #60 fixed. Falls back to fmtDate
// for full timestamps.
...
const JOB_TYPE_LABEL = { outside: 'Outside Job', ancillary: 'Ancillary Work' }
const REPAIR_TYPE_LABEL = { warranty: 'Warranty', paid: 'Paid' }

async function drawServiceHeader(doc, entity, startY) {
  let y = startY
  const [parasLogo, alLogo] = await Promise.all([
    loadImageAsDataURI('/paras-logo.png'),
    getALLogo(),
...
// Entity footer block (For …, stamp, Authorised Signatory) + "generated by".
function drawServiceFooter(doc, { entity, entityCode, generatedByName, generatedByDesignation }, startY, stamp) {
  const isPT = entityCode === 'PT'
  let y = startY + 5
  const rx = PAGE_W - MARGIN

  doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...GRAY_DARK)
  doc.text('For ' + safe(entity.full_name), rx, y, { align: 'right' })
  if (stamp) doc.addImage(stamp, 'PNG', rx - 33, y + 3, 33, 22)
  ...
  // Generated-by block: name + designation, then the full entity name.
  const who = safe(generatedByName) + (generatedByDesignation ? ', ' + generatedByDesignation : '')
  doc.setFontSize(7); doc.setTextColor(...GRAY)
  doc.text('PO generated by: ' + who, MARGIN, sigLineY)
  doc.setFont('helvetica', 'bold')
  doc.text(safe(entity.full_name), MARGIN, sigLineY + 4)
  void isPT
}

export async function generateServicePoPdf(data) {
  const { poNumber, date, entity, entityCode, generatedByName, generatedByDesignation, job } = data
  if (!entity) throw new Error('Cannot generate PO: entity is missing')
  if (!job) throw new Error('Cannot generate PO: job is missing')

  const isPT = entityCode === 'PT'
  const isAW = job.job_type === 'ancillary' && job.repair_type === 'warranty'

  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  let y = await drawServiceHeader(doc, entity, MARGIN)

  // Page-break guard for the free-flowing (non-autoTable) body.
  const ensure = (needed) => { if (y + needed > 270) { doc.addPage(); y = MARGIN } }

  // Title + PO No / Date
  doc.setFont('helvetica', 'bold'); doc.setFontSize(13); doc.setTextColor(...GRAY_DARK)
  doc.text(isAW ? 'WARRANTY REQUEST' : 'PURCHASE ORDER', PAGE_W / 2, y, { align: 'center' })
  y += 7
  doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5)
  doc.text('PO No: ' + safe(poNumber), MARGIN, y)
  doc.text('DATE : ' + fmtDateOnly(date), PAGE_W - MARGIN, y, { align: 'right' })
  y += 6
  doc.setDrawColor(...GRAY_LIGHT); doc.setLineWidth(0.2)
  doc.line(MARGIN, y, PAGE_W - MARGIN, y)
  y += 6

  // Helper: labelled field row with wrapping; skips empty values.
  const field = (label, value, { skipEmpty = true, emptyText = '—' } = {}) => {
    if (skipEmpty && (value == null || value === '')) return
    const display = (value == null || value === '') ? emptyText : String(value)
    const valLines = doc.splitTextToSize(display === '' ? ' ' : display, CONTENT_W - 38)
    ensure(valLines.length * 5 + 2)
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...GRAY_DARK)
    doc.text(label + ' :', MARGIN, y)
    doc.setFont('helvetica', 'normal')
  ...
  const para = (text, { size = 9, gap = 5 } = {}) => {
    const lines = doc.splitTextToSize(String(text), CONTENT_W)
    ensure(lines.length * (size * 0.5) + gap)
    doc.setFont('helvetica', 'normal'); doc.setFontSize(size); doc.setTextColor(...GRAY_DARK)
    doc.text(lines, MARGIN, y)
    y += lines.length * (size * 0.5) + gap
  }

  if (isAW) {
    // ── Warranty-request letter ──────────────────────────────────────
    // Recipient: vendor (service company) on one line, then "Authorized Dealer: <OEM>"
    // on the next, italicised (e.g. "Authorized Dealer: Bosch").
    doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...GRAY_DARK)
    doc.text('To,', MARGIN, y); y += 5
    if (job.vendor_name_snapshot) {
      doc.setFont('helvetica', 'bold'); doc.text(safe(job.vendor_name_snapshot), MARGIN, y); y += 5
    }
    doc.setFont('helvetica', 'italic')
    doc.text(`Authorized Dealer: ${safe(job.vendor_oem_snapshot, '')}`.trimEnd(), MARGIN, y); y += 5
    doc.setFont('helvetica', 'normal')
    y += 3
    para(`I would like to request you kindly check the ${safe(job.check_item, '____')} as found some issue during observation which detail is given below.`, { gap: 6 })

    // Always render every labelled row (matches the AL warranty-request template),
    // leaving the value blank when not captured.
    const lf = { skipEmpty: false, emptyText: '' }
    field('Customer Name', job.customer_name, lf)
    field('Registration', job.vehicle_registration_no, lf)
    field('Chassis Number', job.chassis_no, lf)
    field('Engine No', job.engine_no, lf)
    field('Km /HRS', job.km_hrs, lf)
    field('Date of sale', fmtDateOnly(job.date_of_sale), lf)
    field('Model', job.model, lf)
    field('Complaint Date', fmtDateOnly(job.complaint_date), lf)
    field('Complaint', job.complaint, lf)
    field('Make', job.make, lf)
    field('Serial No', job.serial_no, lf)
    field('Qty', job.qty, lf)
    y += 3
    para('Kindly consider the job under warranty.', { gap: 8 })

    doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...GRAY_DARK)
    ensure(20)
    doc.text('Yours Faithfully,', MARGIN, y); y += 6
    doc.setFont('helvetica', 'bold'); doc.text(safe(entity.full_name), MARGIN, y); y += 5
    doc.setFont('helvetica', 'normal')
    doc.text('Authorized Dealer - Ashok Leyland', MARGIN, y); y += 5
    const footAddr = doc.splitTextToSize(safe(entity.address), CONTENT_W)
    doc.text(footAddr, MARGIN, y); y += footAddr.length * 5
  } else {
    // ── Conventional vendor purchase order ───────────────────────────
    doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...GRAY_DARK)
    doc.text('To,', MARGIN, y); y += 5
    doc.setFont('helvetica', 'bold'); doc.text(safe(job.vendor_name_snapshot, 'Vendor'), MARGIN, y); y += 6
    field('Vendor Address', job.vendor_address_snapshot)
    field('Vendor GSTIN', job.vendor_gstin_snapshot)
    y += 2
    field('Customer Name', job.customer_name, { skipEmpty: false })
    field('Vehicle Reg. No', job.vehicle_registration_no, { skipEmpty: false })
    field('Job Type', JOB_TYPE_LABEL[job.job_type] || job.job_type, { skipEmpty: false })
    field('Repair Type', REPAIR_TYPE_LABEL[job.repair_type] || job.repair_type, { skipEmpty: false })
    field('Job Description', job.job_description)
    field('Ancillary Ref', job.ancillary_ref)
    field('Material Out', fmtDateOnly(job.material_out_date))
    field('Material Returned', fmtDateOnly(job.material_return_date))
    y += 2
    para('Kindly carry out the work detailed above for the referenced vehicle.', { gap: 6 })
  }

  y += 4
  const stamp = await loadImageAsDataURI(isPT ? '/pt-stamp.png' : '/al-stamp.png')
  ensure(34)
  drawServiceFooter(doc, { entity, entityCode, generatedByName, generatedByDesignation }, y, stamp)

  doc.save('PO_' + String(poNumber || 'service').replace(/[-/]/g, '_') + '.pdf')
}
