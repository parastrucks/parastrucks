...
async function verify(
  ...
  // Look up the caller's portal profile — now also includes the new 4-axis
  // columns so future HR-Manager-same-entity gates (Phase 6c.1 follow-up PR)
  // can enforce without an extra round-trip.
  const { data: prof } = await admin
    .from("users")
    .select("id, role, permission_level, entity_id, department_id, is_active, full_name")
    .eq("id", u.user.id)
    .maybeSingle()
  ...
  try {
    switch (action) {
      case "create": {
        ...
        const adminErr = rejectAdminTier(p.role, p.permission_level)
        ...
        const { error: pErr } = await admin.from("users").insert({
          ...
          email: p.email.trim(),
          // Legacy (writes to satisfy existing NOT NULL / CHECK constraints
          // and keep Sidebar/BottomNav/Profile readable until their own cutover PR)
          role: p.role,
          entity: p.entity,
          brand: p.brand || null,
      ...
      case "updateProfile": {
        ...
        // Whitelist updatable columns — no one can change id, email, is_active
        // via this path. Covers both legacy text columns (for back-compat
        // readers) and the new 4-axis UUID columns.
        const allowed = [
          "full_name",
          // Legacy
          "role",
        ...
        const adminErr = rejectAdminTier(
          clean.role as string | null | undefined,
