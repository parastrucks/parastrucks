...
Needs Node and the repo's `xlsx` dependency (SheetJS 0.20.3, already in `package.json`).
Python is not installed on this machine — these are Node, deliberately.

```bash
node scripts/phase10/inventory.js
```
...
|---|---|
| `inventory.js` | Sheet inventory: row counts, formula counts, cell-comment counts per sheet |
| `a_claims.js` | Duplicate chassis, cross-file conflict rates (CTC 26.5% etc.), the FY multi-layout break, the DEALERS column mislabel |
| `b_it_formulas.js` | INVOICE TRACKER money formulas, the per-column formula-variance scan, the comment census, qty |
| `c_sale_g.js` | Sale-side formulas (**TCS basis**, total, CRETEM), text-in-money columns, settlement/G-series numbers, VIN decode, DSE slab |
| `d_followup.js` | Three-way retention relationship, empirical VIN month mapping, the SRS incentive override |

## The one that matters most

`c_sale_g.js` is what caught round 1's inverted TCS formula: `tally_bill × 1%` matches
